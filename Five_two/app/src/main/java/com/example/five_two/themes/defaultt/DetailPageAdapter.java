...
import com.example.five_two.pages.BindPage;
import com.example.five_two.pages.DetailPage;
import com.example.five_two.pages.dialog.ErrorDialog;
import com.example.five_two.pages.dialog.ProgressDialog;
import com.example.five_two.pickers.ImagePicker;
import com.example.five_two.pickers.ImagePicker.OnImageGotListener;
import com.example.five_two.pickers.SingleValuePicker.Choice;
import com.example.five_two.themes.defaultt.components.RollImageView;
import com.example.five_two.themes.defaultt.components.SingleChoiceView;
import com.example.five_two.themes.defaultt.components.SingleLineView;
			...
			public void onClick(View v) {
				...
				builder.setOnImageGotListener(new OnImageGotListener() {
					public void onOmageGot(Bitmap bm) {
						ivAvatar.setBitmap(bm);
						ivAvatar.setRound(ResHelper.dipToPx(activity, 38));
					}
...
//							t.printStackTrace();
	...
	private void uploadImage(String output) {
		if (pd != null && pd.isShowing()) {
			pd.dismiss();
		}
		pd = new ProgressDialog.Builder(getPage().getContext(), getPage().getTheme()).show();
		UMSSDK.uploadAvatar(output, new OperationCallback<HashMap<String, Object>>() {
			public void onSuccess(HashMap<String, Object> data) {
				...
				if (urls != null && urls instanceof ArrayList) {
					...
					if (photoUrl != null && photoUrl.length > 0) {
						int resId = ResHelper.getBitmapRes(getPage().getContext(), "umssdk_default_infor_details_avatar");
						ivAvatar.execute(photoUrl[0], resId);
					}
					HashMap<String, Object> map = new HashMap<String, Object>();
					map.put(getPage().getUser().avatarId.getName(), photoId);
					map.put(getPage().getUser().avatar.getName(), photoUrl);
					updateUserInfo(map);
				}
			}
