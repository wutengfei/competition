package com.example.five_two.themes.defaultt.components;
...
import com.example.five_two.pages.FansPage;
import com.example.five_two.pages.FansPage.PageType;
...
public abstract class UserBriefView<P extends Page<P>> extends RelativeLayout {
	...
	private void init(Context context) {
		setBackgroundColor(0xff3C3427);

		RelativeLayout rlAvatar = new RelativeLayout(context);
		rlAvatar.setId(R.id.my_view);
		...
		LinearLayout.LayoutParams lpll = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams
				.MATCH_PARENT);
		lpll.weight = 1;
	...
	public void setUser(User user) {
		...
		String unLogin = getContext().getString(ResHelper.getStringRes(getContext(), "umssdk_default_un_login"));
		this.user = user;
		if (user == null) {
			tvNick.setText(unLogin);
			tvGender.setText("");
			tvArea.setText("");
			ivAvatar.execute(null,ResHelper.getBitmapRes(getContext(), "umssdk_default_avatar"));
			...
			tvNick.setOnClickListener(null);
		}
		if (!user.nickname.isNull()) {
			if (user.nickname.get().length() > 12) {
				tvNick.setText(user.nickname.get().substring(0, 6) + "..." + user.nickname.get().substring(user
						.nickname.get().length() - 6));
			} else {
