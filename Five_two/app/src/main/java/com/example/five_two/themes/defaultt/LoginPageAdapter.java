package com.example.five_two.themes.defaultt;
...
import com.example.five_two.pages.LoginPage;
import com.example.five_two.pages.RegisterPage;
import com.example.five_two.pages.VcodeLoginPage;
import com.example.five_two.pages.dialog.ErrorDialog;
import com.example.five_two.pages.dialog.ProgressDialog;
import com.example.five_two.themes.defaultt.components.PasswordView;
import com.example.five_two.themes.defaultt.components.PhoneView;
import com.example.five_two.themes.defaultt.components.TitleView;
...
public class LoginPageAdapter extends DefaultThemePageAdapter<LoginPage> implements OnClickListener {
	...
	private void initPage(final Activity activity) {
		...
		RelativeLayout.LayoutParams lprl = new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT, ResHelper
				.dipToPx(activity, 45));
		lprl.addRule(RelativeLayout.CENTER_IN_PARENT);
		rl.addView(tvLogin, lprl);

		tvGoRegister = new TextView(activity);
		tvGoRegister.setId(R.id.my_view2);
		...
		tvVcodeLogin = new TextView(activity);
		tvVcodeLogin.setId(R.id.my_view3);
		tvVcodeLogin.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 14);
		tvVcodeLogin.setTextColor(0xff979797);
		tvVcodeLogin.setText(ResHelper.getStringRes(activity, "umssdk_default_vcode_login"));
		tvVcodeLogin.setGravity(Gravity.CENTER);
		tvVcodeLogin.setOnClickListener(this);
		lprl = new RelativeLayout.LayoutParams(LayoutParams.WRAP_CONTENT, ResHelper.dipToPx(activity, 45));
		lprl.addRule(RelativeLayout.CENTER_HORIZONTAL);
		lprl.addRule(RelativeLayout.ABOVE, tvLogin.getId());
		rl.addView(tvVcodeLogin, lprl);
	...
	private void initSocialLogin(Activity activity, LinearLayout llPage, SocialNetwork[] sns) {
		...
		vSep.setBackgroundColor(0xff3C3427);
		RelativeLayout.LayoutParams lprl = new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT, ResHelper
				.dipToPx(activity, 1));
		lprl.addRule(RelativeLayout.CENTER_IN_PARENT);
	...
	public void onClick(View v) {
		if (v.equals(tvLogin)) {
			...
			if (TextUtils.isEmpty(password) || password.length() <= 0) {
				Toast.makeText(getPage().getContext(), ResHelper.getStringRes(getPage().getContext(),
						"umssdk_default_password_tip"),Toast.LENGTH_SHORT).show();
				return;
			}
			if (pd != null && pd.isShowing()) {
				pd.dismiss();
		...
		} else if (v.equals(tvVcodeLogin)) {
			VcodeLoginPage vcodeLoginPage = new VcodeLoginPage(getPage().getTheme());
			vcodeLoginPage.showForResult(v.getContext(), null, new FakeActivity() {
				public void onResult(HashMap<String, Object> data) {
					if (data != null) {
						getPage().finishOnSuccess((User) data.get("me"));
					}
